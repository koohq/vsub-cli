...
* `-b, --bilingual`: 原語と翻訳文を上下併記したバイリンガル字幕を生成。
* `--bilingual-order <order>`: バイリンガル字幕の並び順（`original-first` または `target-first`。デフォルト: `original-first`）。
...
* `--burn`: 生成された字幕を動画自体に焼き込んで出力 (hardsub)。
